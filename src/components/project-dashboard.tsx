...
export function ProjectDashboard({ project, projectSlug, ideas, role }: { project: Project; projectSlug: string; ideas: Idea[]; role: string }) {
  ...
  return <main className="min-h-screen bg-negro">
    <div className="mx-auto max-w-7xl px-5 py-10 md:px-10">
      <section className="mb-10 flex flex-wrap items-end justify-between gap-8 border-b border-blanco-10 pb-10">
        <div>
          <p className="eyebrow">[PROJECT_STATUS: ACTIVE] · {ROLE_LABEL[roleKey]}</p>
      ...
      <section aria-label="Flujo en fases" className="mb-10">
        ...
        <div className="grid gap-px border-2 border-blanco bg-blanco sm:grid-cols-2 lg:grid-cols-5">
      ...
      <section className="mb-12 grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
        ...
        <div className="border-2 border-mostaza bg-mostaza/5 p-5 sm:p-6">
      ...
      <section>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {allIdeas.slice(0, 6).map((idea) => <Link key={idea.id} href={`/${projectSlug}/ideas/${idea.id}`} className="brutal-panel group">
            <div className="mb-6 flex justify-between gap-3"><span className="mono-label text-mostaza">[{idea.code ?? 'NUEVA'} · {idea.content_type === 'organic' ? 'ORGÁNICO' : 'PAUTA'}]</span><StatusBadge status={idea.status}/></div>
