...
export function ProjectDashboard({ project, projectSlug, ideas, role }: { project: Project; projectSlug: string; ideas: Idea[]; role: string }) {
  ...
  return <main className="min-h-screen bg-negro"><div className="mx-auto max-w-7xl px-5 py-10 md:px-10"><section className="mb-12 flex flex-wrap items-end justify-between gap-8 border-b border-blanco-10 pb-10"><div><p className="eyebrow">[PROJECT_STATUS: ACTIVE] · {role.toUpperCase()}</p><h1 className="display-title">{project.name}<br/><em>OPERACIÓN.</em></h1><p className="mt-5 max-w-2xl text-base leading-8 text-blanco-60">{project.description ?? 'Cada idea conserva su referencia, decisión de cliente, responsable y estado de producción.'}</p></div><div className="flex flex-wrap gap-4"><Link href={`/${projectSlug}/ideas`} className="btn-brutal">VER BANCO →</Link><Link href={`/${projectSlug}/ideas/nueva`} className="btn-brutal-mostaza">+ NUEVA IDEA</Link></div></section>
    <RoleCommandCenter projectSlug={projectSlug} ideas={allIdeas} defaultRole={role}/>
