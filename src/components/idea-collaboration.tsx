...
export function IdeaCollaboration({ projectSlug, ideaId }: { projectSlug: string; ideaId: string }) {
  ...
  async function submitComment(event: React.FormEvent) {
    ...
    if (!text.trim()) return;
    setBusy(true);
    const { error } = await addComment({ ideaId, body: text.trim(), roleLabel: role });
    setBusy(false);
  ...
  async function openAsset(asset: IdeaAsset) {
    if (PUBLIC_MODE) { setNotice('Descarga disponible solo para cuentas autorizadas.'); return; }
  ...
  return <section className="mt-8 border-t-2 border-blanco pt-8" aria-labelledby="collaboration-title">
    ...
    {PUBLIC_MODE && <p className="mb-5 border-2 border-mostaza bg-mostaza/10 p-4 font-mono text-[10px] leading-5 text-blanco">[MODO PÚBLICO · SOLO LECTURA] El hilo de decisiones y el historial de entregas son visibles. Comentar, resolver o subir archivos requiere una cuenta autorizada.</p>}
    <div className="space-y-6">
      <div className="border-2 border-blanco bg-blanco-05 p-4 sm:p-6">
        ...
        <div className="space-y-4">
          {visible.map((comment) => <article key={comment.id} className={`border-l-2 p-3 ${comment.resolved ? 'border-blanco-20 opacity-60' : 'border-fucsia'}`}><div className="flex flex-wrap justify-between gap-2 font-mono text-[10px]"><span className="text-mostaza">{comment.author} · {comment.role}</span><span className="text-blanco-40">{comment.createdAt}</span></div><p className="mt-2 text-sm leading-6 text-blanco-60">{comment.text}</p>{!PUBLIC_MODE && <button onClick={() => toggleResolved(comment)} className="mt-3 font-mono text-[10px] text-orquidea underline">{comment.resolved ? 'REABRIR' : 'MARCAR RESUELTO'}</button>}</article>)}
        ...
        {!PUBLIC_MODE && <form onSubmit={submitComment} className="mt-5 border-t border-blanco-20 pt-5"><div className="mb-3 flex flex-col gap-2 sm:flex-row"><select value={role} onChange={(event) => setRole(event.target.value)} className="input-brutal sm:w-44"><option>RR ALIADOS</option><option>CLIENTE</option><option>MODELO</option><option>CÁMARA</option><option>EDITOR</option><option>PAUTA</option></select><input value={text} onChange={(event) => setText(event.target.value)} className="input-brutal flex-1" placeholder="Escribe una decisión, duda o ajuste..." /></div><button disabled={busy} className="btn-brutal w-full sm:w-auto" type="submit">PUBLICAR COMENTARIO →</button></form>}
      </div>
      <div className="border-2 border-mostaza bg-mostaza/5 p-5 sm:p-7">
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(280px,.7fr)]">
          ...
          <div className={`border-l-2 border-mostaza/50 pl-5 ${PUBLIC_MODE ? 'lg:col-span-2' : ''}`}>
