    ...
    <div className="space-y-6">
      <div className="border-2 border-blanco bg-blanco-05 p-4 sm:p-6">
        ...
        <div className="space-y-4">
          {visible.map((comment) => <article key={comment.id} className={`border-l-2 p-3 ${comment.resolved ? 'border-blanco-20 opacity-60' : 'border-fucsia'}`}><div className="flex flex-wrap justify-between gap-2 font-mono text-[10px]"><span className="text-mostaza">{comment.author} · {comment.role}</span><span className="text-blanco-40">{comment.createdAt}</span></div><p className="mt-2 text-sm leading-6 text-blanco-60">{comment.text}</p><button onClick={() => toggleResolved(comment)} className="mt-3 font-mono text-[10px] text-orquidea underline">{comment.resolved ? 'REABRIR' : 'MARCAR RESUELTO'}</button></article>)}
