...
export function IdeaCollaboration({ projectSlug, ideaId }: { projectSlug: string; ideaId: string }) {
  ...
  return <section className="mt-8 border-t-2 border-blanco pt-8" aria-labelledby="collaboration-title">
    ...
    <div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
      ...
      <div className="border-2 border-mostaza bg-mostaza/5 p-4 sm:p-6"><p className="mono-label text-mostaza">VERSIONES Y ARCHIVOS</p><p className="mt-3 text-sm leading-6 text-blanco-60">Centraliza referencia, guion, crudo y entregables. Cada carga deja una versión; nunca reemplaza la anterior.</p><label className="mt-5 block"><span className="mono-label mb-2 block text-mostaza">// TIPO DE ENTREGA</span><select value={assetStage} onChange={(event) => setAssetStage(event.target.value)} className="input-brutal"><option>REFERENCIA / BRIEF</option><option>GUIÓN</option><option>CONTENIDO CRUDO</option><option>EDICIÓN V1</option><option>EDICIÓN V2</option><option>EDICIÓN FINAL</option><option>PUBLICACIÓN / EVIDENCIA</option></select></label><label className="mt-4 flex min-h-28 cursor-pointer flex-col items-center justify-center border-2 border-dashed border-mostaza p-4 text-center hover:bg-mostaza/10"><input type="file" className="sr-only" onChange={addAsset} accept="image/*,video/*,.pdf,.doc,.docx" /><span className="font-display text-lg font-bold text-mostaza">+ CARGAR {assetStage}</span><span className="mt-2 font-mono text-[10px] text-blanco-40">PDF · VIDEO · IMAGEN · GUIÓN</span></label>{assetNotice && <p role="status" className="mt-4 border border-mostaza bg-mostaza/10 p-3 font-mono text-[10px] leading-5 text-blanco">{assetNotice}</p>}<div className="mt-5 space-y-2">{assets.map(asset => <div key={asset.id} className="border-b border-blanco-20 py-3 font-mono text-[10px]"><div className="flex items-center justify-between gap-2"><span className="truncate text-blanco-60">{asset.kind} // {asset.name}</span><span className="shrink-0 text-mostaza">{asset.version}</span></div><span className="mt-1 block text-blanco-40">{asset.stage} · {asset.createdAt}</span></div>)}{assets.length === 0 && <p className="font-mono text-[10px] text-blanco-40">AÚN NO HAY ARCHIVOS LOCALES</p>}</div></div>
    </div>
  </section>;
}
